...
export const ANSWER_FINAL_STANDINGS_SEASONS = Object.freeze(Array.from({ length: FINAL_SEASON_MAX - SEASON_MIN + 1 }, (_, index) => SEASON_MIN + index));
...
export const ANSWER_TEMPLATE_REGISTRY_CONTRACT = deepFreeze({
  ...
  race_season_finishing_position_h2h: {
    ...
    semantic: 'ordered two-driver final-season race finishing-position comparison over shared events with two recorded numeric positions'
  },
  qualifying_season_position_h2h: {
    variables: { season: finalSeasonConstraint, driver_a_id: driverIdConstraint, driver_b_id: driverIdConstraint },
...
export function materializeAnswerTemplate(templateId: AnswerTemplateId, variables: unknown): F1QLProgram {
  ...
  } else if (templateId === 'driver_career_official_summary') {
    root = {
      op: 'aggregate',
      input: { op: 'filter', input: { op: 'source', source: 'standings' }, where: { season: [...ANSWER_FINAL_STANDINGS_SEASONS], driver_id: scoped.driver_id as string } },
      group_by: ['driver_id'],
      measures: [
  ...
  } else if (templateId === 'qualifying_season_position_h2h') {
    root = {
  ...
  } else if (templateId === 'race_event_finishing_position_comparison') {
    root = {
