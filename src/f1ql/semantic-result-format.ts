...
export const SEMANTIC_RESULT_FORMAT_VERSION = 'semantic-result-format-v26' as const;
...
function buildSemanticPlanResult(execution: SemanticExecutionFormattingBinding): BuiltSemanticPlanResult {
  ...
  const qualifyingCountRankingContract = isQualifyingPositionCountRankingContract(
    core.root.count, core.root.input.keys, project, branches, sources, columns
  );
  ...
  if (qualifyingCountRankingContract && rows.length === 0) {
    throw new SemanticResultFormatError('Qualifying count ranking source evidence was incomplete');
  }
  ...
  else if (!finalStandingsContract && execution.has_more_rows && !qualifyingCountRankingContract) {
...
// eslint-disable-next-line complexity
function isQualifyingPositionCountRankingContract(
  rowLimit: number,
  ordering: readonly PlannedCoreSortKey[],
  project: PlannedCoreProjectNode,
  branches: ReturnType<typeof inputBranches>,
  ...
  return rowLimit === 10 && sources.length === 1 && source?.id === 'qualifying_classification' &&
    branches.length === 1 && branch.input.source_id === 'qualifying_classification' &&
    branch.predicates.length === 1 && seasonPredicate?.concept.source_id === 'qualifying_classification' &&
    seasonPredicate.concept.concept_id === 'season' && seasonPredicate.operator === 'eq' &&
    typeof seasonPredicate.value === 'number' && Number.isSafeInteger(seasonPredicate.value) &&
    source.scope.season_min !== null && seasonPredicate.value >= source.scope.season_min &&
    source.scope.final_season_through !== null && seasonPredicate.value <= source.scope.final_season_through &&
    aggregate !== undefined && aggregate.group_by.length === 1 &&
    aggregate.group_by[0].source_id === 'qualifying_classification' &&
    aggregate.group_by[0].concept_id === 'driver_id' && aggregate.measures.length === 1 &&
    measure?.source_id === 'qualifying_classification' && measure.concept_id === 'qualifying_position' &&
    measure.function === 'count' && measure.as === 'count_qualifying_position' &&
    project.outputs.length === 2 && project.outputs[0].kind === 'concept' &&
    project.outputs[0].concept.source_id === 'qualifying_classification' &&
    project.outputs[0].concept.concept_id === 'driver_id' && project.outputs[0].as === 'driver_id' &&
    project.outputs[1].kind === 'aggregate' &&
    project.outputs[1].measure_as === 'count_qualifying_position' &&
    project.outputs[1].as === 'count_qualifying_position' &&
    ...
    columns[1].id === 'count_qualifying_position' && columns[1].kind === 'aggregate' &&
    columns[1].aggregation === 'count' && columns[1].physical_type === 'integer' &&
    columns[1].nullable === false && ordering.length === 2 &&
    ordering[0].output_id === 'count_qualifying_position' && ordering[0].direction === 'desc' &&
