...
export const ANSWER_QUESTION_CONTRACT_VERSION = 'answer-question-v20' as const;
...
export type AnswerQuestionMetricCue = 'points' | 'official_leader' | 'latest_recorded' | 'official_season_summary' | 'official_career_summary' | 'race_finishing_position_h2h' | 'qualifying_position_h2h' | 'date';
...
function determineOutcome(
  ...
  if ((sources.length > 0 || results.length > 0 || metrics.some(metric => metric.value === 'race_finishing_position_h2h' || metric.value === 'qualifying_position_h2h')) && years.length === 0) {
