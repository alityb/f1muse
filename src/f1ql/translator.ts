...
- official_event_mean_compare for exactly two drivers over all eligible official completed laps in one race; only metric official_non_deleted_non_pit_event_mean_v1 is supported
...
Required named official event-mean program: {"version":1,"root":{"op":"official_event_mean_compare","metric":"official_non_deleted_non_pit_event_mean_v1","season":2022,"event_name":"Belgium","driver_a_id":"Verstappen","driver_b_id":"Alonso"}}
...
For either official comparison, preserve the user's literal event and driver references; deterministic code resolves them. Never invent a season, event, driver, aggregate, or lap bound. Otherwise use canonical lowercase hyphenated driver IDs. Never invent an unsupported F1QL operation.
