...
const EVENT_METADATA_SCALAR_FAMILY_PROMPT = 'For explicit catalog-grounded requests that name race date, circuit identifier, or event name from final YYYY event metadata at exactly one round or named event, emit one event_metadata candidate with only the requested date, circuit_id, or event_name output. When both race date and event name are requested, emit exactly date then event_name in that order; this exact pair is the only supported combined event-metadata projection. A circuit identifier is a raw identifier, not a circuit, venue, or Grand Prix name; an event name is recorded source text, not a circuit or venue name. Do not extend these event-metadata rules to date plus circuit_id, event_name plus circuit_id, all three fields, multiple events, season-wide requests, user-supplied limits, latest-recorded or interim scope, or qualifying, practice, or sprint dates.';
...
export const SEMANTIC_CANDIDATE_PROVIDER_SYSTEM_PROMPT =
  `${SEMANTIC_CANDIDATE_EFFECTIVE_SYSTEM_PROMPT} ${STANDINGS_SUMMARY_FAMILY_PROMPT} ${EVENT_METADATA_SCALAR_FAMILY_PROMPT} ${RACE_POSITION_RANKING_FAMILY_PROMPT} ${QUALIFYING_POSITION_RANKING_FAMILY_PROMPT}`;
