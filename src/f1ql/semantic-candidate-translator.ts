...
const SELECTED_RACE_POSITION_COUNT_FAMILY_PROMPT = 'For explicit catalog-grounded requests that name driver and count of finishing position from final YYYY race classification for exactly two to four specific drivers, emit exactly driver_id followed by COUNT(finishing_position), group by event_classification.driver_id, and use one event_classification.driver_id in filter grounded by every selected driver. Use comparison count, no order_by, and no caller limit. This selected per-driver rule takes precedence over both the pooled scalar race count and the zero-driver top-10 count-ranking rule. Count only non-null retained recorded finishing positions; a selected driver retained only with null positions has factual count zero. Do not extend this rule to zero, one, or five-or-more drivers, pooled scalar wording, qualifying classification, round or event scope, status, position, or team filters, ranking, top-N, caller limits, comparisons, alternate grouping or aggregation, broader or reordered outputs, repeated clauses, latest-recorded, interim, or return-all and universe claims. Selected per-driver qualifying-position counts remain unauthorized.';
