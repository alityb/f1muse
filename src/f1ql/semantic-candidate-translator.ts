...
const QUALIFYING_COUNT_RANKING_FAMILY_PROMPT = 'For the exact zero-driver request to show top 10 drivers by count of qualifying position in one final YYYY qualifying classification season, emit exactly driver_id followed by COUNT(qualifying_position), group by driver_id, rank by the count descending, and set the limit to exactly 10. Use no entity or row filter. Count only non-null recorded qualifying-position rows; a retained driver with only null positions has count zero. This is not qualifying appearances, events, poles, top-ten finishes, or complete participation. Driver identity only stabilizes equal counts and the top-10 cutoff. Do not extend this rule to any other limit, no limit, selected drivers, race classification, round or event scope, position or status filters, alternate grouping or aggregation, additional outputs, comparison or delta, latest-recorded 2026, or interim scope.';
