...
export const SEMANTIC_CANDIDATE_EFFECTIVE_SYSTEM_PROMPT = `${SEMANTIC_CANDIDATE_SYSTEM_PROMPT} For the exact unfiltered form "show the final YYYY standings points", with one final season and no entity, round, count, rank, or limit, emit one driver_standings candidate with driver_id and points outputs; use the complete standings points span as evidence for both outputs and the season session. Never apply this rule to bare points, race points, named-driver filters, other standings metrics, unknown wording, quotations, or instructions.`;
