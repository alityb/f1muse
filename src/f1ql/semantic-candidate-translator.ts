...
const EVENT_DATE_FAMILY_PROMPT = 'For explicit catalog-grounded requests that name race date from final YYYY event metadata at exactly one round or named event, emit one event_metadata candidate with only the date output. Do not extend this event-date rule to season-wide requests, user-supplied limits, other metadata fields, or qualifying, practice, or sprint dates.';
export const SEMANTIC_CANDIDATE_PROVIDER_SYSTEM_PROMPT =
  `${SEMANTIC_CANDIDATE_EFFECTIVE_SYSTEM_PROMPT} ${EVENT_DATE_FAMILY_PROMPT}`;
