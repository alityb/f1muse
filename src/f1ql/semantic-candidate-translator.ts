...
export class SemanticCandidateProposalAdapter {
  ...
  async propose(
    ...
    let raw: string;
    try {
      raw = await this.model.complete(SEMANTIC_CANDIDATE_SYSTEM_PROMPT, providerRequest, signal);
    } catch (error) {
