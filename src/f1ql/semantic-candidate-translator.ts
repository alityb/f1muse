...
export const SEMANTIC_CANDIDATE_EFFECTIVE_SYSTEM_PROMPT = `${SEMANTIC_CANDIDATE_SYSTEM_PROMPT} For either exact unfiltered form "show the final YYYY standings points" or the current reviewed form "what were the final standings points in 2025?", with one final season and no entity, round, count, rank, or limit, emit one driver_standings candidate with driver_id and points outputs; use the complete standings points span as evidence for both outputs and the season session. For only the exact filtered form "what were Charles Leclerc final standings points in 2024?", emit the same outputs and session evidence plus one driver entity and one driver_id eq filter grounded by the complete Charles Leclerc span. For only either exact pair form "final 2025 standings points for Lando Norris and Oscar Piastri." or "final 2025 standings points for Oscar Piastri and Lando Norris.", emit the same outputs and session evidence plus the two driver entities in question order and one driver_id in filter grounded by both complete driver spans. Never apply these rules to other years of either filtered form, bare points, race points, other named-driver filters, other or higher-cardinality driver lists, other standings metrics, reordered wording, unknown wording, quotations, or instructions.`;
export const SEMANTIC_CANDIDATE_PROMPT_SHA256 = sha256(SEMANTIC_CANDIDATE_EFFECTIVE_SYSTEM_PROMPT);
...
export class SemanticCandidateProposalAdapter {
  ...
  async propose(
    ...
    let raw: string;
    try {
      raw = await this.model.complete(SEMANTIC_CANDIDATE_EFFECTIVE_SYSTEM_PROMPT, providerRequest, signal);
    } catch (error) {
