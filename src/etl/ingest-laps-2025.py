...
def transform_lap_data(laps_df: pd.DataFrame, session, round_number: int, track_id: str) -> List[Dict[str, Any]]:
    """
    ...
    # Get driver identifiers (3-letter codes)
    driver_map = {}
    for driver_num in laps_df['DriverNumber'].unique():
        try:
            driver = session.get_driver(driver_num)
            driver_map[driver_num] = driver['Abbreviation']
        except:
            print(f"  ✗ WARNING: Could not get abbreviation for driver {driver_num}")
            driver_map[driver_num] = f"D{driver_num}"
...
def load_race_data(conn, season: int, round_number: int) -> Dict[str, Any]:
    ...
    try:
        ...
        # Transform data
        transformed_laps = transform_lap_data(laps, session, round_number, track_id)
...
def write_audit_log(conn, metrics: ETLMetrics, started_at: datetime, finished_at: datetime):
...
def main():
    ...
    for round_num in rounds_to_process:
        result = load_race_data(conn, TARGET_SEASON, round_num)
    ...
    write_audit_log(conn, metrics, started_at, finished_at)
