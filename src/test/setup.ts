...
export async function setupTestDatabase(
  ...
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS pace_v2_lap_rebuild (
      rebuild_version TEXT NOT NULL, season INTEGER NOT NULL, round INTEGER NOT NULL, track_id TEXT NOT NULL,
      driver_id TEXT NOT NULL, session_type TEXT NOT NULL, lap_number INTEGER NOT NULL, stint_id INTEGER NOT NULL,
      stint_lap_index INTEGER NOT NULL, lap_time_seconds NUMERIC, is_valid_lap BOOLEAN NOT NULL, is_pit_lap BOOLEAN NOT NULL,
      is_out_lap BOOLEAN NOT NULL, is_in_lap BOOLEAN NOT NULL, clean_air_flag BOOLEAN NOT NULL, compound TEXT,
      tyre_age_laps INTEGER, methodology_version TEXT NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS pace_v2_rebuild_audit (
      rebuild_version TEXT NOT NULL, season INTEGER NOT NULL, round INTEGER NOT NULL, session_type TEXT NOT NULL,
      ...
      replacement_fact_fingerprint TEXT NOT NULL, original_fact_row_count INTEGER NOT NULL, replacement_fact_row_count INTEGER NOT NULL,
      canonical_driver_fingerprint TEXT NOT NULL, canonical_driver_count INTEGER NOT NULL, methodology_version TEXT NOT NULL,
      PRIMARY KEY (rebuild_version, season, round, session_type)
    );
  `);
  ...
  await pool.query(`
    ...
    WITH approved_rebuilds AS (
      SELECT l.* FROM pace_v2_lap_rebuild l JOIN pace_v2_rebuild_audit a ON a.rebuild_version = l.rebuild_version AND a.season = l.season AND a.round = l.round AND a.session_type = l.session_type
      ...
      SELECT l.season, l.round, l.track_id, l.driver_id, l.session_type, l.lap_number, l.lap_time_seconds, l.is_valid_lap, l.is_pit_lap, l.is_in_lap, l.is_out_lap, l.clean_air_flag, l.compound, l.tyre_age_laps, l.methodology_version FROM approved_rebuilds l
      UNION ALL
      ...
      FROM approved_replacements l WHERE NOT EXISTS (SELECT 1 FROM pace_v2_rebuild_audit a WHERE a.rebuild_version = 'fastf1_complete_race_v1' AND a.season = l.season AND a.round = l.round AND a.session_type = l.session_type)
      UNION ALL
      SELECT l.season, l.round, l.track_id, l.driver_id, l.session_type, l.lap_number, l.lap_time_seconds, l.is_valid_lap, l.is_pit_lap, l.is_in_lap, l.is_out_lap, l.clean_air_flag, l.compound, l.tyre_age_laps, l.methodology_version
      FROM laps_normalized_v2 l
      WHERE NOT EXISTS (SELECT 1 FROM pace_v2_rebuild_audit a WHERE a.rebuild_version = 'fastf1_complete_race_v1' AND a.season = l.season AND a.round = l.round AND a.session_type = l.session_type)
        AND NOT EXISTS (SELECT 1 FROM pace_v2_replacement_audit a WHERE a.replacement_version = 'nat_pit_flags_v1' AND a.season = l.season AND a.round = l.round AND a.session_type = l.session_type)
    )
    SELECT
      l.season,
      l.round,
