...
function getOperation(body: unknown): 'aggregate' | 'rank' | 'pace_delta' | 'pace_summary' | 'event_classification' | 'official_lap_window_median_compare' | 'official_event_mean_compare' | 'race_season_finishing_position_h2h' | 'race_event_finishing_position_comparison' | 'qualifying_season_position_h2h' | 'official_driver_results_comparison' | 'driver_career_wins_by_circuit' | 'driver_season_qualifying_p1_count' | 'driver_career_qualifying_p1_count' | 'driver_season_qualifying_top_ten_count' | 'season_qualifying_top_ten_ranking' | 'invalid' {
  const operation = (body as { root?: { op?: unknown } })?.root?.op;
  return operation === 'aggregate' || operation === 'rank' || operation === 'pace_delta' || operation === 'pace_summary' || operation === 'event_classification' || operation === 'official_lap_window_median_compare' || operation === 'official_event_mean_compare' || operation === 'race_season_finishing_position_h2h' || operation === 'race_event_finishing_position_comparison' || operation === 'qualifying_season_position_h2h' || operation === 'official_driver_results_comparison' || operation === 'driver_career_wins_by_circuit' || operation === 'driver_season_qualifying_p1_count' || operation === 'driver_career_qualifying_p1_count' || operation === 'driver_season_qualifying_top_ten_count' || operation === 'season_qualifying_top_ten_ranking'
    ? operation
    : 'invalid';
}
