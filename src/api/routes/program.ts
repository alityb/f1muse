...
function getOperation(body: unknown): 'aggregate' | 'rank' | 'pace_delta' | 'pace_summary' | 'event_classification' | 'official_lap_window_median_compare' | 'official_event_mean_compare' | 'race_season_finishing_position_h2h' | 'qualifying_season_position_h2h' | 'official_driver_results_comparison' | 'driver_career_wins_by_circuit' | 'invalid' {
  const operation = (body as { root?: { op?: unknown } })?.root?.op;
  return operation === 'aggregate' || operation === 'rank' || operation === 'pace_delta' || operation === 'pace_summary' || operation === 'event_classification' || operation === 'official_lap_window_median_compare' || operation === 'official_event_mean_compare' || operation === 'race_season_finishing_position_h2h' || operation === 'qualifying_season_position_h2h' || operation === 'official_driver_results_comparison' || operation === 'driver_career_wins_by_circuit'
    ? operation
    : 'invalid';
}
