...
async function main(): Promise<void> {
  ...
  if (answerEvaluationManifest.length !== 112) {
  ...
  try {
    ...
    if (uniqueCaseCount !== answerEvaluationManifest.length || uniqueCaseCount !== 112 || artifact.observations.length !== expectedObservationCount) {
