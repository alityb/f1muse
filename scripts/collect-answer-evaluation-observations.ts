...
async function main(): Promise<void> {
  ...
  if (answerEvaluationManifest.length !== 114) {
  ...
  try {
    ...
    if (uniqueCaseCount !== answerEvaluationManifest.length || uniqueCaseCount !== 114 || artifact.observations.length !== expectedObservationCount) {
