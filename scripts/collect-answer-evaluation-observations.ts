...
async function main(): Promise<void> {
  ...
  if (answerEvaluationManifest.length !== 113) {
  ...
  try {
    ...
    if (uniqueCaseCount !== answerEvaluationManifest.length || uniqueCaseCount !== 113 || artifact.observations.length !== expectedObservationCount) {
