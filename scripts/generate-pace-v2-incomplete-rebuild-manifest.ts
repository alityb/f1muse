...
import { createPaceV2IncompleteRebuildManifest, fingerprintDriverIds, PACE_V2_INCOMPLETE_REBUILD_ROUNDS } from '../src/etl/pace-v2-incomplete-rebuild';
...
export async function generatePaceV2IncompleteRebuildManifest(pool: QueryPool) {
  ...
  try {
    ...
    const [persisted, canonical] = await Promise.all([
      client.query('SELECT season, round, track_id, driver_id, session_type, lap_number, stint_id, stint_lap_index, lap_time_seconds, is_valid_lap, is_pit_lap, is_out_lap, is_in_lap, clean_air_flag, compound, tyre_age_laps, methodology_version FROM laps_normalized_v2 WHERE season = 2026 AND round = ANY($1::int[]) AND session_type = \'R\' ORDER BY round, driver_id, lap_number', [PACE_V2_INCOMPLETE_REBUILD_ROUNDS]),
      client.query('SELECT DISTINCT r.round, rd.driver_id FROM race r JOIN race_data rd ON rd.race_id = r.id AND LOWER(rd.type) IN (\'race\', \'race_result\') WHERE r.year = 2026 AND r.round = ANY($1::int[]) ORDER BY r.round, rd.driver_id', [PACE_V2_INCOMPLETE_REBUILD_ROUNDS])
    ]);
    const rows = persisted.rows.map(fact);
    const rounds = PACE_V2_INCOMPLETE_REBUILD_ROUNDS.map((round) => {
      ...
      if (!original.length || !driverIds.length || driverIds.length <= persistedIds.size || [...persistedIds].some((id) => !driverIds.includes(id)) || original.some((row) => row.methodology_version !== 'clean_air_gap_2_0s_v1')) throw new Error(`FAIL_CLOSED: round ${round} is not an approved incomplete canonical-coverage rebuild candidate`);
...
if (require.main === module) { const pool = new Pool({ connectionString: configuration(), ssl: { rejectUnauthorized: false }, max: 1 }); generatePaceV2IncompleteRebuildManifest(pool).then((value) => process.stdout.write(`${JSON.stringify(value)}\n`)).catch(() => { process.stdout.write('{"status":"refused","error":"pace_v2_incomplete_rebuild_manifest_failed"}\n'); process.exitCode = 1; }).finally(() => pool.end()); }
