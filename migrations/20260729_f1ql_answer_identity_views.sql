-- Phase 7 answer-only identity contract. Role creation and grants are intentionally separate.
...
WITH (security_barrier = true) AS
SELECT d.id::text AS driver_id, identity_value.identity
...
WHERE identity_value.identity IS NOT NULL
UNION ALL
SELECT d.id::text AS driver_id, da.alias::text AS identity
FROM public.driver AS d
...
WITH (security_barrier = true) AS
SELECT sed.year AS season, sed.driver_id::text AS driver_id, 'entrant'::text AS participation_source
FROM public.season_entrant_driver AS sed
WHERE sed.test_driver = false
UNION ALL
SELECT dse.year AS season, dse.driver_id::text AS driver_id, 'legacy_fallback'::text AS participation_source
...
COMMENT ON VIEW f1ql.answer_driver_identity IS 'Minimal answer-only canonical driver and alias identity values.';
COMMENT ON VIEW f1ql.answer_season_participation IS 'Answer-only entrant participation with explicit legacy fallback provenance.';
