...
describe('answer bounds', () => {
  it.each(['race_season_finishing_position_h2h', 'qualifying_season_position_h2h'] as const)('charges two complete season branches and requests one %s row', template => {
    ...
    expect(estimateAnswerWork(h2h, capability(h2h))).toEqual({ version: 'answer-work-v9', units: 60, requested_rows: 1 });
  });
  ...
  it('bounds a driver season summary to one row and 36 work units', () => {
    ...
    expect(estimateAnswerWork(summary, capability(summary))).toEqual({ version: 'answer-work-v9', units: 36, requested_rows: 1 });
  });
  ...
  it('charges all 76 final seasons while bounding a driver career summary to one row', () => {
    ...
    expect(estimateAnswerWork(summary, capability(summary))).toEqual({ version: 'answer-work-v9', units: 107, requested_rows: 1 });
  });
  ...
  it('charges both 76-season career-win sources and bounds grouped circuits', () => {
    ...
    expect(estimateAnswerWork(wins, capability(wins))).toEqual({ version: 'answer-work-v9', units: 172, requested_rows: 100 });
  });
  ...
  it('bounds the pinned three-driver final ranking to three rows', () => {
    ...
    expect(estimateAnswerWork(ranking, capability(ranking))).toEqual({ version: 'answer-work-v9', units: 38, requested_rows: 3 });
  });
  ...
  it('charges the complete official comparison foundation and requests one row', () => {
    ...
    expect(estimateAnswerWork(comparison, capability(comparison))).toEqual({ version: 'answer-work-v9', units: 122, requested_rows: 1 });
  });
  ...
  it('charges two branches and one row for one event comparison', () => {
    ...
    expect(estimateAnswerWork(comparison, capability(comparison))).toEqual({ version: 'answer-work-v9', units: 2, requested_rows: 1 });
  });
