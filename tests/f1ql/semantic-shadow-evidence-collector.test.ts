...
describe('WP8 semantic shadow evidence collector', () => {
  ...
  it('binds all 19 reviewed questions and outcomes to the committed compositional snapshot', () => {
    const reviewed = assertCompleteReviewedCompositionalCorpus(compositionalRegressionCorpusInput, snapshot());
    expect(reviewed.corpus.cases).toHaveLength(19);
    expect(reviewed.snapshot.cases).toHaveLength(19);
