...
import { ANSWER_EVALUATION_TRANSLATION_TIMEOUT_MS, collectAnswerObservations, createAnswerObservationSigningHelper, getAnswerEvaluationManifestHash, isVerifiedAnswerObservationArtifact, parseAnswerObservationArtifact, signAnswerObservationArtifact, validateAnswerObservationArtifact, verifyAnswerObservationArtifact } from '../../src/f1ql/answer-observations';
...
describe('answer observation artifacts', () => {
  ...
  it('collects deterministic question outcomes and semantic proofs without execution', async () => {
    ...
    const results: AnswerTranslationResult[] = [
      ...
      { type: 'clarification_required', reason: 'metric_ambiguous' }
    ];
    let translated = 0;
    let paced = 0;
    let now = 0;
    ...
    const artifact = await collectAnswerObservations(cases, provider, {
      ...
      translate: async () => { now += 10; return { result: results[translated++], timedOut: false }; },
    ...
    expect(paced).toBe(2);
    expect(pacingDelays).toEqual([990]);
    expect(artifact).toMatchObject({ version: 3, provider, evaluation: { key_id: keyId, algorithm: 'Ed25519' }, manifest: { case_count: 4, sha256: getAnswerEvaluationManifestHash(cases) } });
    ...
    expect(artifact.observations[1]).not.toHaveProperty('translation_latency_ms');
  ...
  it('remains structurally non-executing and formatter-free', () => {
    ...
    expect(answerEvaluationManifest).toHaveLength(79);
    expect(command.match(/!== 79/g)).toHaveLength(2);
  });
});
