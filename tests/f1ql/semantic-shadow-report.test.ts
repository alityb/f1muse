...
describe('WP8 semantic shadow report', () => {
  it.skipIf(!supportsRetainedV2())('aggregates raw and Railway records without retaining raw content or question hashes', () => {
    ...
    expect(report.repetition).toEqual({
      expected_question_groups: 43, expected_repetitions_per_group: 3, expected_attempts: 129,
  ...
  it.skipIf(!supportsRetainedV2())('requires all 43 question groups with exactly three stable semantic repetitions', () => {
    const corpusSha256 = HASH('c');
    const runSha256 = HASH('d');
    const questionHashes = Array.from({ length: 43 }, (_, index) =>
    ...
    expect(report.repetition).toEqual({
      expected_question_groups: 43, expected_repetitions_per_group: 3, expected_attempts: 129,
      question_groups: 43, repeated_groups: 43, complete_groups: 43, incomplete_groups: 0,
      overfull_groups: 0, stable_groups: 43, drifted_groups: 0, status: 'pass'
    });
  ...
  it('derives complete reviewed candidate, work, dual, and pre/post-provider oracle fields', () => {
    ...
    expect(requirements.cases).toHaveLength(43);
