...
describe('WP8 semantic shadow report', () => {
  it.skipIf(!supportsRetainedV2())('aggregates raw and Railway records without retaining raw content or question hashes', () => {
    ...
    expect(report.repetition).toEqual({
      expected_question_groups: 36, expected_repetitions_per_group: 3, expected_attempts: 108,
  ...
  it.skipIf(!supportsRetainedV2())('requires all 36 question groups with exactly three stable semantic repetitions', () => {
    const corpusSha256 = HASH('c');
    const runSha256 = HASH('d');
    const questionHashes = Array.from({ length: 36 }, (_, index) =>
    ...
    expect(report.repetition).toEqual({
      expected_question_groups: 36, expected_repetitions_per_group: 3, expected_attempts: 108,
      question_groups: 36, repeated_groups: 36, complete_groups: 36, incomplete_groups: 0,
      overfull_groups: 0, stable_groups: 36, drifted_groups: 0, status: 'pass'
    });
  ...
  it('derives complete reviewed candidate, work, dual, and pre/post-provider oracle fields', () => {
    ...
    expect(requirements.cases).toHaveLength(36);
