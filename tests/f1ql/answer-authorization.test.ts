...
describe('one-time answer execution authorization', () => {
  ...
  it('issues a branded, frozen, audience/deployment/release-bound short-lived authorization', async () => {
    ...
    expect(authorization).toMatchObject({
      ...
      active_versions: { authorization: 'answer-authorization-v14', release_attestation: 5 }
    });
  ...
  it('issues distinct authority for the authenticated internal canary principal', async () => {
    ...
    expect(authorization).toMatchObject({ version: 8, principal_class: 'internal_canary' });
