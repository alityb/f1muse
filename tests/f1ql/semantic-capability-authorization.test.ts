...
const POSITIVE_PROFILE_CASES = {
  'semantic-single-source-v1': [({ year }: PositiveProfileInput): PositiveProfileCase => ({
    ...
    question: `List race date and circuit identifier from round ${round} of final ${year} event metadata.`,
    entity_names: []
  }), ({ year, round }: PositiveProfileInput): PositiveProfileCase => ({
    question: `List event name and circuit identifier from round ${round} of final ${year} event metadata.`,
    entity_names: []
  }), ({ year, round }: PositiveProfileInput): PositiveProfileCase => ({
    question: `List race date, event name, and circuit identifier from round ${round} of final ${year} event metadata.`,
    entity_names: []
  }), ({ year, round, candidate_count, selected_index }: PositiveProfileInput): PositiveProfileCase => ({
...
describe('semantic complete-interaction capability authorization', () => {
  ...
  it.each([
    ...
    'List race date and circuit identifier from round 1 of latest recorded 2026 event metadata.',
    'List event name and circuit identifier from round 1 of latest recorded 2026 event metadata.',
    'List race date, event name, and circuit identifier from round 1 of latest recorded 2026 event metadata.'
  ...
  it('binds only the exact top-10 recorded qualifying-position count ranking', async () => {
    const question = 'Show top 10 drivers by count of qualifying position in final 2025 qualifying classification.';
    const proof = await semanticProof(question, []);
    const authorization = authorizeSemanticPlanCapability({
      proof,
      profile_id: 'semantic-single-source-v1',
    ...
    expect(authorization.interaction).toMatchObject({
      ...
      sort_bindings: ['count_qualifying_position:desc:last', 'driver_id:asc:last'],
      entity_count: 0,
      event_count: 0,
      season_count: 1,
      season_values: [2025],
    ...
    expect(authorization.result_collection).toMatchObject({
      returned_row_limit: 10,
      completeness_probe_rows: 1,
      observed_row_limit: 11
    });
  });

  it('binds only the exact top-10 recorded race finishing-position count ranking', async () => {
    const question = 'Show top 10 drivers by count of finishing position in final 2025 race classification.';
    const proof = await semanticProof(question, []);
    const authorization = authorizeSemanticPlanCapability({
      proof,
      profile_id: 'semantic-single-source-v1',
    ...
    expect(authorization.interaction).toMatchObject({
      ...
      sort_bindings: ['count_finishing_position:desc:last', 'driver_id:asc:last'],
      entity_count: 0,
      event_count: 0,
      season_count: 1,
      season_values: [2025],
