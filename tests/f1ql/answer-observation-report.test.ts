...
describe('answer observation reporting', () => {
  it('emits aggregate template, semantic, link, latency, and timeout gates without private identifiers', () => {
    ...
    expect(report.artifact.observations).toBe(242);
    expect(report.reliability).toMatchObject({ answerable_cases: 69, required_observations: 207, supplied_observations: 207, complete_cases: 69, status: 'pass' });
    for (const field of ['action', 'reason', 'template_id', 'program_hash'] as const) {
      expect(report.reliability[field]).toEqual({ exact_cases: 69, drift_cases: 0 });
    }
  ...
  it.each([
    ...
    expect(report.reliability[field].exact_cases).toBe(68);
  ...
  it('fails identical repeated wrong values without calling them drift', () => {
    ...
    const report = buildAnswerObservationReport(answerEvaluationManifest, answerMetamorphicGroups, artifact, artifactHash);
    expect(report.reliability.reason).toEqual({ exact_cases: 68, drift_cases: 0 });
  ...
  it('marks a canonical missing repetition insufficient rather than drift', () => {
    ...
    const report = buildAnswerObservationReport(answerEvaluationManifest, answerMetamorphicGroups, artifact, artifactHash);
    expect(report.reliability).toMatchObject({ supplied_observations: 206, complete_cases: 68, status: 'insufficient' });
  ...
  it('retains explicit historical artifact compatibility but marks proof evidence insufficient', () => {
    ...
    expect(report.reliability).toMatchObject({ supplied_observations: 69, complete_cases: 0, status: 'insufficient' });
