...
describe('semantic query candidates and independent evidence', () => {
  ...
  it('enumerates explicit row scope and aggregate ranking semantics', () => {
    ...
    const raceRanking = candidateEvidence(
      RACE_POSITION_RANK_QUESTION,
      driverNames.map(text => ({ type: 'driver', span: span(RACE_POSITION_RANK_QUESTION, text) }))
    ).candidates[0];
    expect(raceRanking).toMatchObject({
      outputs: [
        { kind: 'concept', concept: { source_id: 'event_classification', concept_id: 'driver_id' } },
        { kind: 'concept', concept: { source_id: 'event_classification', concept_id: 'finishing_position' } }
      ],
      filters: [{ kind: 'entity', operator: 'in', entity_indices: [0, 1, 2] }],
      comparison: { relation: 'rank' },
      order_by: [{ output_index: 1, direction: 'asc' }]
    ...
    const reorderedRanking = candidateEvidence(
      reorderedQuestion,
      driverNames.map(text => ({ type: 'driver', span: span(reorderedQuestion, text) }))
    ).candidates[0];
  ...
  it.each([2, 3, 4] as const)('enumerates exact selected per-driver race counts for %i drivers', cardinality => {
    const names = ['Lando Norris', 'Oscar Piastri', 'George Russell', 'Charles Leclerc'].slice(0, cardinality);
    const question = cardinality === 2
      ? SELECTED_RACE_COUNT_QUESTION
      : `Show driver and count of finishing position for ${names.join(', ')} in final 2025 race classification.`;
  ...
  it('enumerates only the exact unfiltered per-driver race count shape', () => {
    const candidate = candidateEvidence(UNFILTERED_RACE_DRIVER_COUNT_QUESTION).candidates[0];
    expect(candidate).toMatchObject({
      outputs: [
        { kind: 'concept', concept: { source_id: 'event_classification', concept_id: 'driver_id' } },
        {
  ...
  it.each([
    ...
    'Show count of finishing position per team in final 2025 race classification.',
  ...
  it.each([
    ...
    'Show driver and count of finishing position 1 for Lando Norris and Oscar Piastri in final 2025 race classification.',
    'Show driver, team, and count of finishing position for Lando Norris and Oscar Piastri in final 2025 race classification.',
    ...
    'Compare driver and count of finishing position for Lando Norris and Oscar Piastri in final 2025 race classification.',
    'Show driver and maximum finishing position for Lando Norris and Oscar Piastri in final 2025 race classification.',
    ...
    'Show driver and count of finishing position grouped by driver for Lando Norris and Oscar Piastri in final 2025 race classification.',
    'Show driver, finishing position, and count of finishing position for Lando Norris and Oscar Piastri in final 2025 race classification.',
    ...
    'Show driver and count of finishing position for Lando Norris and Oscar Piastri in interim 2025 race classification.',
  ...
  it.each([
    'Rank drivers Max Verstappen by finishing position from round 1 of final 2025 race classification.',
    'Rank drivers Max Verstappen and Lando Norris by finishing position from final 2025 race classification.',
    'Show top 2 drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification.',
    'Rank drivers Max Verstappen and Lando Norris by race points from round 1 of final 2025 race classification.',
    'Rank drivers Max Verstappen and Lando Norris by finishing position and qualifying position from round 1 of final 2025 race classification.',
    'Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of latest recorded 2026 race classification.'
  ])('rejects race rankings outside the selected-driver recorded-position family: %s', question => {
  ...
  it('rejects unknown language appended to a selected-driver race ranking', () => {
    const question = 'Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification secretly.';
  ...
  it.each([
    '"Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification."',
    "'Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification.'",
    '‘Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification.’',
    '«Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification.»',
    '‛Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification.',
    "Rank drivers Max Verstappen and Lando Norris by'finishing position'from round 1 of final 2025 race classification.",
    '¬ Rank drivers Max Verstappen and Lando Norris by finishing position from round 1 of final 2025 race classification.'
  ])('rejects quoted or symbol-negated race ranking instructions: %s', question => {
    expect(enumerateSemanticQueries(question, ['Max Verstappen', 'Lando Norris'].map(text => ({
      type: 'driver', span: span(question, text)
    })))).toMatchObject({ type: 'abstention', reason: 'unknown_language' });
  });
  ...
  it.each([
  ...
  ] as const)('enumerates selected-driver race metadata projection: %s', (projection, metadataOutputs) => {
    const question = `List driver, finishing position, and ${projection} for Lando Norris and Oscar Piastri from round 1 of final 2025 race classification and event metadata.`;
    ...
    expect(candidate.outputs.map(output => output.concept.concept_id)).toEqual([
      'driver_id', 'finishing_position', ...metadataOutputs
    ]);
  ...
  it.each([
    'List driver, finishing position, circuit identifier, race date, and event name for Lando Norris from round 1 of final 2025 race classification and event metadata.',
    'List finishing position, driver, event name, circuit identifier, and race date for Lando Norris from round 1 of final 2025 race classification and event metadata.'
  ])('canonicalizes reordered selected-driver join outputs: %s', question => {
    const candidate = candidateEvidence(question, [{
      type: 'driver', span: span(question, 'Lando Norris')
    }]).candidates[0];
    expect(candidate.outputs.map(output => output.concept.concept_id)).toEqual([
      'driver_id', 'finishing_position', 'date', 'event_name', 'circuit_id'
    ]);
  });

  it.each([
    'List top 1 driver, finishing position, and race date for Lando Norris from round 1 of final 2025 race classification and event metadata.',
    'List driver, finishing position, race points, and race date for Lando Norris from round 1 of final 2025 race classification and event metadata.',
    'List driver and finishing position for Lando Norris from round 1 of final 2025 race classification and event metadata.',
    'List driver, finishing position, and race date for Lando Norris from round 1 of latest recorded 2026 race classification and event metadata.',
    'List driver, finishing position, and race date for Lando Norris from round 1 of interim 2025 race classification and event metadata.',
    'List driver, finishing position, and race date for Lando Norris from final 2025 race classification and event metadata.',
    'Rank Lando Norris by finishing position and show race date from round 1 of final 2025 race classification and event metadata.',
    'Show count of finishing position and race date for Lando Norris from round 1 of final 2025 race classification and event metadata.',
    'List driver, finishing position, and race date for Max Verstappen, Lando Norris, Oscar Piastri, George Russell, and Charles Leclerc from round 1 of final 2025 race classification and event metadata.'
  ])('rejects an unsupported selected-driver race metadata expansion: %s', question => {
    const names = ['Max Verstappen', 'Lando Norris', 'Oscar Piastri', 'George Russell', 'Charles Leclerc']
      .filter(name => question.includes(name));
    expect(enumerateSemanticQueries(question, names.map(text => ({
      type: 'driver', span: span(question, text)
  ...
  it('maps only the two reviewed unfiltered final standings-points forms', () => {
    for (const question of [DEV_POINTS_QUESTION, IID_POINTS_ALL_QUESTION]) {
      const evidence = candidateEvidence(question);
      const standingsPoints = span(question, 'standings points');
      ...
      expect(admitSemanticQueryCandidates(
        { version: 2, candidates: [omittedDriver] }, question, evidence
      )).toEqual({ type: 'abstention', reason: 'provider_candidate_not_enumerated' });
    }

    expect(enumerateSemanticQueries('What were the final 2025 standings points?'))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
    expect(enumerateSemanticQueries('What were the final standings points in 2025'))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
    expect(enumerateSemanticQueries('What were the final standings points in 2025.'))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
    expect(enumerateSemanticQueries('What were the final standings points in 2024?'))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
    expect(enumerateSemanticQueries('What were the final standings points in 2025 secretly?'))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
  ...
  it('maps only the two exact reviewed Lando and Oscar standings-points orders', () => {
    for (const [question, names] of [
      [PAIR_POINTS_QUESTION, ['Lando Norris', 'Oscar Piastri']],
      [REVERSED_PAIR_POINTS_QUESTION, ['Oscar Piastri', 'Lando Norris']]
    ] as const) {
      const driverSpans = names.map(name => span(question, name));
      ...
      expect(evidence.candidates).toEqual([expect.objectContaining({
        ...
        filters: [{
          kind: 'entity', concept: { source_id: 'driver_standings', concept_id: 'driver_id' },
          operator: 'in', entity_indices: [0, 1], evidence: driverSpans
        }],
        group_by: [],
        order_by: []
      })]);
    ...
    for (const question of [
      ...
      'Final 2025 standings points for Oscar Piastri or Lando Norris.',
    ...
    expect(enumerateSemanticQueries(PAIR_POINTS_QUESTION))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
    expect(enumerateSemanticQueries(REVERSED_PAIR_POINTS_QUESTION))
      .toMatchObject({ type: 'abstention', reason: 'unknown_language' });
  });
  ...
  it('does not collapse output alternatives or conflicting cardinality cues', () => {
    ...
    const eventCircuit = 'List circuit identifier from round 1 of final 2025 event metadata.';
    expect(enumerateSemanticQueries(eventCircuit)).toMatchObject({
      type: 'candidate_set',
      candidates: [expect.objectContaining({
        outputs: [expect.objectContaining({
          kind: 'concept', concept: { source_id: 'event_metadata', concept_id: 'circuit_id' }
        })]
      })]
    });
    const namedEventCircuit = 'List circuit id from final 2025 event metadata at Monaco.';
    ...
    for (const [question, expected] of [
      ['List circuit identifier and race date from round 1 of final 2025 event metadata.', ['date', 'circuit_id']],
      ['List circuit identifier and event name from round 1 of final 2025 event metadata.', ['event_name', 'circuit_id']],
      ['List circuit identifier, event name, and race date from round 1 of final 2025 event metadata.',
        ['date', 'event_name', 'circuit_id']]
    ] as const) {
      const evidence = enumerateSemanticQueries(question);
      expect(evidence).toMatchObject({ type: 'candidate_set', candidates: [expect.any(Object)] });
      expect((evidence as any).candidates[0].outputs.map((output: any) => output.concept.concept_id))
        .toEqual(expected);
    }
    for (const question of [
      'List race date and circuit identifier from final 2025 event metadata at Monaco.',
      'List event name and circuit identifier from final 2025 event metadata at Monaco.',
      'List race date, event name, and circuit identifier from final 2025 event metadata at Monaco.'
    ]) {
      expect(enumerateSemanticQueries(question, [{ type: 'event', span: span(question, 'Monaco') }]))
        .toMatchObject({ type: 'candidate_set', candidates: [expect.any(Object)] });
    }
    for (const question of [
      'List round and circuit identifier from round 1 of final 2025 event metadata.',
      'List season and event name from round 1 of final 2025 event metadata.',
      'List race date, event name, circuit identifier, season, and round from round 1 of final 2025 event metadata.',
      'List circuit identifier and round from round 1 of final 2025 event metadata.',
      'List event name with season from round 1 of final 2025 event metadata.'
    ]) {
    ...
    for (const question of [
      'List round 1 race date and circuit identifier from final 2025 event metadata.',
      'List round #1 race date and circuit identifier from final 2025 event metadata.',
      'List first round race date and circuit identifier from final 2025 event metadata.',
      'Show season 2025 round 1 event name from event metadata.'
    ]) {
      expect(enumerateSemanticQueries(question)).toMatchObject({
        type: 'candidate_set', candidates: [expect.any(Object)]
      });
    }
