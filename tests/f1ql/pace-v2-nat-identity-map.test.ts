...
const rows = PACE_V2_NAT_REPLACEMENT_ROUNDS.flatMap((round) => [
  { round, race_track_id: `track_${round}`, v2_track_id: `track_${round}`, driver_id: `driver_${round}_a`, race_driver_id: `driver_${round}_a`, driver_code: 'AAA' },
  { round, race_track_id: `track_${round}`, v2_track_id: `track_${round}`, driver_id: `driver_${round}_b`, race_driver_id: `driver_${round}_b`, driver_code: 'BBB' }
]);
...
describe('NaT FastF1 identity-map generator', () => {
  ...
  it('maps exact FastF1 codes only after read-only v2/race identity reconciliation', async () => {
    ...
    expect(artifact).toMatchObject({ version: 1, source: 'approved_fastf1_identity_map', season: 2026 });
