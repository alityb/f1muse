...
describe('generic proven semantic result formatting', () => {
  ...
  it('formats a complete selected-driver official-position ranking and rejects incomplete rank evidence', async () => {
    ...
    expect(formatted).toMatchObject({
      format_version: 'semantic-result-format-v31',
      answer: {
        headline: 'Final 2025 driver standings result.',
        facts: [
  ...
  it('formats exact selected race-count membership in identity order', async () => {
    ...
    expect(prepared.plan).toMatchObject({
      topology: 'single_source_aggregate', output_grain: ['driver_id'],
      work: { requested_rows: 100 }
    });
    const rows = [
      { driver_id: 'lando-norris', count_finishing_position: 2, [PLANNED_INTEGRITY_FIELD]: true },
      { driver_id: 'oscar-piastri', count_finishing_position: 0, [PLANNED_INTEGRITY_FIELD]: true }
    ];
    const execution = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, rows);
    expect(formatSemanticPlanResult(execution)).toMatchObject({
      format_version: 'semantic-result-format-v31',
      rows: [
        { driver_id: 'lando-norris', count_finishing_position: 2 },
        { driver_id: 'oscar-piastri', count_finishing_position: 0 }
      ],
      metadata: {
        ordering: [{ output_id: 'driver_id', direction: 'asc', nulls: 'last' }],
        coverage: { status: 'sufficient', rows_returned: 2, row_limit: 100 }
    ...
    for (const invalidRows of [
      ...
      [rows[1], rows[0]],
      [rows[0], rows[0]],
      [rows[0], { ...rows[1], driver_id: 'other-driver' }],
      [rows[0], { ...rows[1], count_finishing_position: null }],
      [rows[0], { ...rows[1], count_finishing_position: -1 }],
      [rows[0], { ...rows[1], count_finishing_position: 1.5 }],
      [rows[0], { ...rows[1], count_finishing_position: '0' }],
      [rows[0], { ...rows[1], [PLANNED_INTEGRITY_FIELD]: false }]
    ]) {
      const invalid = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, invalidRows);
      expect(() => formatSemanticPlanResult(invalid)).toThrow(SemanticResultFormatError);
    }
    const truncated = await executeSemanticPlanRowsOffline(
      prepared.proof,
      prepared.profile_id,
      Array.from({ length: 101 }, (_unused, index) => ({
        driver_id: `driver-${String(index).padStart(3, '0')}`,
        count_finishing_position: 1,
        [PLANNED_INTEGRITY_FIELD]: true
      }))
    );
    expect(() => formatSemanticPlanResult(truncated)).toThrow('collection evidence was incomplete');
  });

  it('formats exact selected qualifying-count membership in identity order', async () => {
    const lando = span(SELECTED_QUALIFYING_COUNT, 'Lando Norris');
    const oscar = span(SELECTED_QUALIFYING_COUNT, 'Oscar Piastri');
    const prepared = await prepare(
      SELECTED_QUALIFYING_COUNT,
      [{ type: 'driver', span: lando }, { type: 'driver', span: oscar }],
      [
        { ...lando, candidates: ['lando-norris'], active_candidates: ['lando-norris'] },
        { ...oscar, candidates: ['oscar-piastri'], active_candidates: ['oscar-piastri'] }
    ...
    expect(formatSemanticPlanResult(execution)).toMatchObject({
      format_version: 'semantic-result-format-v31',
      rows: [
        { driver_id: 'lando-norris', count_qualifying_position: 2 },
        { driver_id: 'oscar-piastri', count_qualifying_position: 0 }
      ],
      metadata: {
        ordering: [{ output_id: 'driver_id', direction: 'asc', nulls: 'last' }],
        coverage: { status: 'sufficient', rows_returned: 2, row_limit: 100 }
    ...
    for (const invalidRows of [
      ...
      [rows[0], rows[0]],
      [rows[0], { ...rows[1], driver_id: 'other-driver' }],
      [rows[0], { ...rows[1], count_qualifying_position: null }],
      [rows[0], { ...rows[1], count_qualifying_position: -1 }],
      [rows[0], { ...rows[1], count_qualifying_position: 1.5 }],
      [rows[0], { ...rows[1], count_qualifying_position: '0' }],
      [rows[0], { ...rows[1], [PLANNED_INTEGRITY_FIELD]: false }]
    ]) {
      const invalid = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, invalidRows);
      expect(() => formatSemanticPlanResult(invalid)).toThrow(SemanticResultFormatError);
  ...
  it('requires one through 100 complete ordered unfiltered race driver groups', async () => {
    ...
    expect(formatSemanticPlanResult(execution)).toMatchObject({
      ...
      rows: [
        ...
        { driver_id: 'beta-driver', count_finishing_position: 0 }
      ],
      metadata: {
        ordering: [{ output_id: 'driver_id', direction: 'asc', nulls: 'last' }],
        coverage: { status: 'sufficient', rows_returned: 2, row_limit: 100 }
    ...
    for (const invalidRows of [
      ...
      [{ ...rows[0], count_finishing_position: -1 }],
      [{ ...rows[0], count_finishing_position: 1.5 }],
      [{ ...rows[0], count_finishing_position: Number.MAX_SAFE_INTEGER + 1 }],
      [{ ...rows[0], count_finishing_position: '2' }],
      [{ ...rows[0], driver_id: 'UPPER CASE' }],
      [{ ...rows[0], driver_id: 'driver_id' }],
      [{ ...rows[0], driver_id: '-driver' }],
      [{ ...rows[0], driver_id: 'driver--id' }],
      [{ ...rows[0], driver_id: 'driver id' }],
      [{ ...rows[0], [PLANNED_INTEGRITY_FIELD]: false }]
    ]) {
      const invalid = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, invalidRows);
    ...
    const completeLimit = Array.from({ length: 100 }, (_unused, index) => ({
      driver_id: `driver-${String(index).padStart(3, '0')}`,
      count_finishing_position: index === 0 ? 0 : index,
      [PLANNED_INTEGRITY_FIELD]: true
    }));
    const complete = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, completeLimit);
    expect(formatSemanticPlanResult(complete).rows).toHaveLength(100);
    const truncated = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, [
      ...completeLimit,
      { driver_id: 'driver-100', count_finishing_position: 100, [PLANNED_INTEGRITY_FIELD]: true }
    ]);
    expect(() => formatSemanticPlanResult(truncated)).toThrow('collection evidence was incomplete');
  });

  it('requires one through 100 complete ordered unfiltered qualifying driver groups', async () => {
    ...
    const rows = [
      ...
      { driver_id: 'beta-driver', count_qualifying_position: 0, [PLANNED_INTEGRITY_FIELD]: true },
      { driver_id: 'zulu-driver', count_qualifying_position: 2, [PLANNED_INTEGRITY_FIELD]: true }
    ];
    const execution = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, rows);
    expect(formatSemanticPlanResult(execution)).toMatchObject({
      format_version: 'semantic-result-format-v31',
    ...
    for (const invalidRows of [
      ...
      [{ ...rows[0], count_qualifying_position: -1 }],
      [{ ...rows[0], count_qualifying_position: 1.5 }],
      [{ ...rows[0], count_qualifying_position: Number.MAX_SAFE_INTEGER + 1 }],
      [{ ...rows[0], count_qualifying_position: '1' }],
      [{ ...rows[0], driver_id: 'UPPER CASE' }],
      [{ ...rows[0], driver_id: 'driver_id' }],
      [{ ...rows[0], driver_id: '-driver' }],
    ...
    const completeLimit = Array.from({ length: 100 }, (_unused, index) => ({
      driver_id: `driver-${String(index).padStart(3, '0')}`,
      count_qualifying_position: index === 49 ? 0 : index + 1,
      [PLANNED_INTEGRITY_FIELD]: true
    }));
    const complete = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, completeLimit);
    expect(formatSemanticPlanResult(complete).rows).toHaveLength(100);
    const truncated = await executeSemanticPlanRowsOffline(prepared.proof, prepared.profile_id, [
      ...completeLimit,
      { driver_id: 'driver-100', count_qualifying_position: 1, [PLANNED_INTEGRITY_FIELD]: true }
    ]);
    expect(() => formatSemanticPlanResult(truncated)).toThrow('collection evidence was incomplete');
  });
